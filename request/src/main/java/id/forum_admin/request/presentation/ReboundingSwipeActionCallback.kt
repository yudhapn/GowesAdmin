...
class ReboundingSwipeActionCallback : ItemTouchHelper.SimpleCallback(
    ...
    private fun translateReboundingView(
        ...
        val dragFraction = when {
            dX > 0 -> ln((1 + (dX / swipeDismissDistanceHorizontal)).toDouble()) / ln(3.toDouble())
            else -> ln((1 - (dX / swipeDismissDistanceHorizontal)).toDouble()) / ln(3.toDouble())
        }
        val dragTo = dragFraction * swipeDismissDistanceHorizontal *
                swipeReboundingElasticity
        viewHolder.reboundableView.translationX = when {
            dX > 0 -> dragTo.toFloat()
            else -> -dragTo.toFloat()
        }
    }
